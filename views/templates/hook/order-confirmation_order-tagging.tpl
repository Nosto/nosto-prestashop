...
{if isset($nosto_order) && is_object($nosto_order)}
	<div class="nosto_purchase_order" style="display:none">
		...
		<span class="order_status_code">{$nosto_order->getOrderStatusCode()|escape:'htmlall':'UTF-8'}</span>
		<span class="order_status_label">{$nosto_order->getOrderStatusLabel()|escape:'htmlall':'UTF-8'}</span>
		...
		<div class="buyer">
			<span class="first_name">{$nosto_order->getCustomer()->getFirstName()|escape:'htmlall':'UTF-8'}</span>
			<span class="last_name">{$nosto_order->getCustomer()->getLastName()|escape:'htmlall':'UTF-8'}</span>
			<span class="email">{$nosto_order->getCustomer()->getEmail()|escape:'htmlall':'UTF-8'}</span>
		</div>
		<div class="purchased_items">
			{foreach from=$nosto_order->getPurchasedItems() item=item}
				<div class="line_item">
					...
					<span class="price_currency_code">{$item->getPriceCurrencyCode()|escape:'htmlall':'UTF-8'}</span>
				</div>
			{/foreach}
		</div>
	</div>
